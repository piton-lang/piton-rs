...
| `output` | Optional. Where `piton build` writes renderer output. Defaults to `./dist`. With a framework, the renderer output is only written when `output` or `renderer` is set. |
