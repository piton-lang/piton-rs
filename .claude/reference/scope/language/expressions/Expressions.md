...
Expressions combine values and operators to produce a result. Literal expressions such as 1 + 2 evaluate directly, while expressions that reference variables use braces, such as {a + b}. Evaluation follows Piton’s type and operator rules, resolves forward references, and preserves anchor identity. Unresolved or cyclic references are compiler errors.
...
myVariable: {1 + 2}
```
`myVariable` will be evaluated to 3. Evaluation will of course follow all the rules we’ve previously defined about types as operators; 2 + Hello will evaluate to a string 2Hello.
An important thing to note is that an expression must be wrapped in curly braces, otherwise it'll be interpreted as a [Strings](../types/Strings.md).
...
We encounter a probably intuitive but perhaps less obvious scenario when we use [ComplexTypes](../types/ComplexTypes.md) like [Lists](../types/Lists.md), [Dictionaries](../types/Dictionaries.md), and [Anchors](../types/Anchors.md).
```piton
myList: [1, 2, 3]
...
- [StandardExpression](StandardExpression.md)
- [NumericExpression](NumericExpression.md)
- [ReferenceExpression](ReferenceExpression.md)
- [StringExpression](StringExpression.md)
