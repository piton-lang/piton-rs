...
- A line of prose that begins like a key, such as `and: more` or a mistyped `x::bad: 1`, keeps its own line and ends the paragraph, and no rewrapped line is made to begin like one, so a key written inside prose by mistake stays where the author can see it.
