...
Offering what can be written where the cursor is, and nothing else

## Summary

Completion is decided by where the cursor is, read from the syntax tree, and each place offers only what is valid there. Prose offers nothing, because a popup over a sentence is noise.
...
Wherever a name can be written, in braces, in an interpolation, in a constraint, or after `extends`, every name another module exports that the file cannot see yet is offered after the names in scope. Its detail names the specifier it would come from, and accepting it adds the import in the same edit, as [ImportSpecifiers](ImportSpecifiers.md) describes.
At the start of a top-level line, every keyword another module exports that the file has not brought in is offered, and accepting it adds the `use` line.
Each name is offered once, from its best specifier, and never when the file already has something in scope with that name.
