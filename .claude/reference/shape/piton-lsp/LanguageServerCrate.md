...
This crate implements [LspScope](../../scope/lsp/LspScope.md), and every behaviour it has is a rule in that spec. When a behaviour has to change, change the Piton source under `spec/scope/lsp` first, run `piton build`, and then change the code to match.
